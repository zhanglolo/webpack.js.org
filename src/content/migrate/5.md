...
### Upgrade webpack and its dependencies {#upgradewebpackanditsdependencies}

#### Upgrade webpack 4 to the latest available version {#upgradewebpack4tothelatestavailableversion}
...
#### Upgrade webpack-cli to the latest available version (when used) {#upgradewebpack-clitothelatestavailableversionwhenused}

#### Upgrade all used Plugins and Loaders to the latest available version {#upgradeallused-pluginsand-loaderstothelatestavailableversion}
...
#### Make sure your build has no errors or warnings {#makesureyourbuildhasnoerrorsorwarnings}
...
#### Make sure you are using entry point information from stats {#makesureyouareusingentrypointinformationfromstats}
...
#### Make sure to use `mode` {#makesuretousemode}
...
#### Update outdated options {#updateoutdatedoptions}
...
#### Test webpack 5 compatibility {#testwebpack5compatibility}
...
#### Upgrade webpack version {#upgradewebpackversion}

npm: `npm install webpack@next --dev`

Yarn: `yarn add webpack@next -D`

#### Clean up configuration {#cleanupconfiguration}
...
#### Cleanup the code {#cleanupthecode}
...
#### Cleanup the build code {#cleanupthebuildcode}

- When using `const compiler = webpack(...);`, make sure to close the compiler after using it: `compiler.close();`.

#### Run a single build and follow advises {#runasinglebuildandfollowadvises}
...
#### Turn off ES2015 syntax in runtime code, if necessary {#turnoff-e-s2015syntaxinruntimecodeifnecessary}
...
#### Everything works? {#everythingworks}
...
#### It is not working? {#itisnotworking}
...
#### Something missing in this guide? {#somethingmissinginthisguide}

Please open a Pull Request to help the next person using this guide.

## Changes to internals {#changestointernals}
...
### getOptions method for Loaders {#get-optionsmethodfor-loaders}
